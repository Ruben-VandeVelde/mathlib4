...
This file contains:
* some basic theorems about natural numbers
* extra recursors:
...
Std has a home-baked development of the algebraic and order theoretic theory of `Nat` which, in
...
Less basic uses of `Nat` should however use the typeclass-mediated development. `Data.Nat.Basic` gives
...
namespace Nat
variable {a b c d m n k : Nat} {p q : Nat → Prop}

-- instance nontrivial : Nontrivial Nat := ⟨⟨0, 1, Nat.zero_ne_one⟩⟩
...
theorem succ_pos' : 0 < succ n := succ_pos n
-- #align nat.succ_pos' Nat.succ_pos'

alias succ_inj := succ_inj'
-- #align nat.succ_inj' Nat.succ_inj

-- theorem succ_injective : Injective Nat.succ := @succ.inj
-- #align nat.succ_injective Nat.succ_injective

-- theorem succ_ne_succ : succ m ≠ succ n ↔ m ≠ n := succ_injective.ne_iff
-- #align nat.succ_ne_succ Nat.succ_ne_succ

-- Porting note: no longer a simp theorem, as simp can prove this
theorem succ_succ_ne_one (n : Nat) : n.succ.succ ≠ 1 := by simp
-- #align nat.succ_succ_ne_one Nat.succ_succ_ne_one

@[simp] theorem one_lt_succ_succ (n : Nat) : 1 < n.succ.succ := succ_lt_succ <| succ_pos n
...
theorem le_succ_iff : m ≤ n.succ ↔ m ≤ n ∨ m = n.succ := by
...
theorem lt_add_one_iff : m < n + 1 ↔ m ≤ n := Nat.lt_succ_iff
...
-- This is true reflexively, by the definition of `≤` on Nat,
-- but it's still useful to have, to convince Lean to change the syntactic type.
theorem add_one_le_iff : m + 1 ≤ n ↔ m < n := Iff.rfl
...
theorem pred_eq_sub_one (n : Nat) : pred n = n - 1 := rfl
...
theorem _root_.Classical.not_and_iff_or_not (p q : Prop) : ¬ (p ∧ q) ↔ ¬ p ∨ ¬ q :=
  letI := Classical.propDecidable p
  letI := Classical.propDecidable q
  Decidable.not_and_iff_or_not p q

theorem exists_lt_succ : (∃ m < n + 1, p m) ↔ (∃ m < n, p m) ∨ p n := by
  have := not_congr <| forall_lt_succ (n := n) (p := (¬·) ∘ p)
  simp only [Classical.not_forall] at this
  simp only [exists_prop, Function.comp, Classical.not_not] at this
  simp only [Classical.not_and_iff_or_not, Classical.not_not] at this
  simp only [Classical.not_forall, Classical.not_not, exists_prop] at this
  exact this
...
-- We want to use these two theorems earlier than the theorems simp can prove them with
...
-- matching. These theorems package them back up as typeclass mediated operations.
@[simp] theorem add_def : Nat.add m n = m + n := rfl
...
@[simp] protected theorem add_eq_zero : m + n = 0 ↔ m = 0 ∧ n = 0 := by omega
-- We want to use this theorem earlier than the theorem simp can prove it with
-- attribute [nolint simpNF] Nat.add_eq_zero
...
theorem add_succ_lt_add (hab : a < b) (hcd : c < d) : a + c + 1 < b + d := by
  rw [Nat.add_assoc]; exact Nat.add_lt_add_of_lt_of_le hab (Nat.succ_le_iff.2 hcd)
-- #align nat.add_succ_lt_add Nat.add_succ_lt_add

protected theorem lt_or_le (a b : Nat) : a < b ∨ b ≤ a :=
  if hba : b ≤ a then Or.inr hba else Or.inl <| Nat.lt_of_not_ge hba

protected theorem le_or_lt (a b : Nat) : a ≤ b ∨ b < a :=
  (Nat.lt_or_le b a).symm

theorem le_or_le_of_add_eq_add_pred (h : a + c = b + d - 1) : b ≤ a ∨ d ≤ c := by
  cases Nat.le_or_lt b a with
...
protected theorem lt_sub_iff_add_lt : a < c - b ↔ a + b < c := ⟨add_lt_of_lt_sub, lt_sub_of_add_lt⟩
protected theorem lt_sub_iff_add_lt' : a < c - b ↔ b + a < c := by omega
protected theorem sub_lt_iff_lt_add (hba : b ≤ a) : a - b < c ↔ a < b + c := by omega
protected theorem sub_lt_iff_lt_add' (hba : b ≤ a) : a - b < c ↔ a < c + b := by omega

protected theorem sub_sub_sub_cancel_right (h : c ≤ b) : a - c - (b - c) = a - b := by omega
protected theorem add_sub_sub_cancel (h : c ≤ a) : a + b - (a - c) = b + c := by omega
protected theorem sub_add_sub_cancel (hab : b ≤ a) (hcb : c ≤ b) : a - b + (b - c) = a - c := by omega

theorem lt_pred_iff : a < pred b ↔ succ a < b := Nat.lt_sub_iff_add_lt (b := 1)
-- #align nat.lt_pred_iff Nat.lt_pred_iff

protected theorem sub_lt_sub_iff_right (h : c ≤ a) : a - c < b - c ↔ a < b := by omega
...
@[simp] theorem mul_def : Nat.mul m n = m * n := rfl
...
theorem two_mul_ne_two_mul_add_one : 2 * n ≠ 2 * m + 1 :=
  mt (congrArg (· % 2))
...
protected theorem mul_ne_mul_left (ha : a ≠ 0) : b * a ≠ c * a ↔ b ≠ c := not_congr (Nat.mul_left_inj ha)
-- #align nat.mul_ne_mul_left Nat.mul_ne_mul_left

protected theorem mul_ne_mul_right (ha : a ≠ 0) : a * b ≠ a * c ↔ b ≠ c := not_congr (Nat.mul_right_inj ha)
...
-- TODO: Deprecate
theorem mul_right_eq_self_iff (ha : 0 < a) : a * b = a ↔ b = 1 := mul_eq_left $ ne_of_gt ha
-- #align nat.mul_right_eq_self_iff Nat.mul_right_eq_self_iff

theorem mul_left_eq_self_iff (hb : 0 < b) : a * b = b ↔ a = 1 := mul_eq_right $ ne_of_gt hb
-- #align nat.mul_left_eq_self_iff Nat.mul_left_eq_self_iff

protected theorem le_of_mul_le_mul_right (h : a * c ≤ b * c) (hc : 0 < c) : a ≤ b :=
...
theorem one_lt_mul_iff : 1 < m * n ↔ 0 < m ∧ 0 < n ∧ (1 < m ∨ 1 < n) := by
  constructor <;> intro h
  · by_contra h'
    rw [Classical.not_and_iff_or_not, Classical.not_and_iff_or_not, not_or] at h'
    simp only [Nat.not_lt, le_zero_eq] at h'
    obtain rfl | rfl | h' := h'
    · simp at h
    · simp at h
...
theorem mul_self_le_mul_self (h : m ≤ n) : m * m ≤ n * n := Nat.mul_le_mul h h
-- #align nat.mul_self_le_mul_self Nat.mul_self_le_mul_self

theorem mul_lt_mul'' (hac : a < c) (hbd : b < d) : a * b < c * d :=
  Nat.mul_lt_mul_of_lt_of_le hac (Nat.le_of_lt hbd) $ by omega

theorem mul_self_lt_mul_self (h : m < n) : m * m < n * n := mul_lt_mul'' h h
-- #align nat.mul_self_lt_mul_self Nat.mul_self_lt_mul_self

theorem mul_self_le_mul_self_iff : m * m ≤ n * n ↔ m ≤ n :=
...
theorem mul_self_lt_mul_self_iff : m * m < n * n ↔ m < n := by
...
theorem mul_self_inj : m * m = n * n ↔ m = n := by simp [Nat.le_antisymm_iff, mul_self_le_mul_self_iff]
...
theorem div_lt_self' (a b : Nat) : (a + 1) / (b + 2) < a + 1 :=
...
theorem div_lt_iff_lt_mul' (hb : 0 < b) : a / b < c ↔ a < c * b := by
...
theorem div_lt_one_iff (hb : 0 < b) : a / b < 1 ↔ a < b := by simp only [← Nat.not_le, one_le_div_iff hb]
-- #align nat.div_lt_one_iff Nat.div_lt_one_iff

-- @[gcongr]
protected theorem div_le_div_right (h : a ≤ b) : a / c ≤ b / c :=
...
protected theorem eq_mul_of_div_eq_left (H1 : b ∣ a) (H2 : a / b = c) : a = c * b := by
  rw [Nat.mul_comm, Nat.eq_mul_of_div_eq_right H1 H2]
-- #align nat.eq_mul_of_div_eq_left Nat.eq_mul_of_div_eq_left

protected theorem mul_div_cancel_left' (Hd : a ∣ b) : a * (b / a) = b := by
...
@[simp]
protected theorem div_left_inj (hda : d ∣ a) (hdb : d ∣ b) : a / d = b / d ↔ a = b := by
...
theorem div_mul_div_comm : b ∣ a → d ∣ c → (a / b) * (c / d) = (a * c) / (b * d) := by
  rintro ⟨x, rfl⟩ ⟨y, rfl⟩
...
theorem eq_zero_of_le_half (h : n ≤ n / 2) : n = 0 := eq_zero_of_le_div (Nat.le_refl _) h
-- #align nat.eq_zero_of_le_half Nat.eq_zero_of_le_half

theorem le_half_of_half_lt_sub (h : a / 2 < a - b) : b ≤ a / 2 := by
...
theorem half_le_of_sub_le_half (h : a - b ≤ a / 2) : a / 2 ≤ b := by
...
protected theorem div_le_of_le_mul' (h : m ≤ k * n) : m / k ≤ n := by
...
protected theorem div_le_self' (m n : Nat) : m / n ≤ m := by
...
protected theorem pow_lt_pow_left (h : a < b) : ∀ {n : Nat}, n ≠ 0 → a ^ n < b ^ n
  | 1, _ => by simpa
...
protected theorem pow_le_pow_iff_left {n : Nat} (hn : n ≠ 0) : a ^ n ≤ b ^ n ↔ a ≤ b where
...
theorem lt_pow_self (ha : 1 < a) : ∀ n : Nat, n < a ^ n
  | 0 => by simp
  | n + 1 =>
    calc
...
theorem lt_two_pow (n : Nat) : n < 2 ^ n := lt_pow_self (by decide) n
-- #align nat.lt_two_pow Nat.lt_two_pow

theorem one_le_pow (n m : Nat) (h : 0 < m) : 1 ≤ m ^ n := by simpa using Nat.pow_le_pow_of_le_left h n
-- #align nat.one_le_pow Nat.one_le_pow

theorem one_le_pow' (n m : Nat) : 1 ≤ (m + 1) ^ n := one_le_pow n (m + 1) (succ_pos m)
-- #align nat.one_le_pow' Nat.one_le_pow'
...
protected theorem div_pow (h : a ∣ b) : (b / a) ^ c = b ^ c / a ^ c := by
...
This section is here due to dependencies -- the theorems here require some of the theorems
...
@[simp]
theorem rec_zero {C : Nat → Sort _} (h0 : C 0) (h : ∀ n, C n → C (n + 1)) : Nat.rec h0 h 0 = h0 := rfl
-- #align nat.rec_zero Nat.rec_zero

@[simp] theorem rec_add_one {C : Nat → Sort _} (h0 : C 0) (h : ∀ n, C n → C (n + 1)) (n : Nat) :
...
@[elab_as_elim]
def leRecOn' {C : Nat → Sort _} : ∀ {m}, n ≤ m → (∀ ⦃k⦄, n ≤ k → C k → C (k + 1)) → C n → C m
...
theorem leRecOn_succ {C : Nat → Sort _} {n m} (h1 : n ≤ m) {h2 : n ≤ m + 1} {next} (x : C n) :
...
theorem leRecOn_succ' {C : Nat → Sort _} {n} {h : n ≤ n + 1} {next : ∀ {k}, C k → C (k + 1)} (x : C n) :
    (leRecOn h next x : C (n + 1)) = next x := by rw [leRecOn_succ (Nat.le_refl n), leRecOn_self]
-- #align nat.le_rec_on_succ' Nat.leRecOn_succ'

theorem leRecOn_trans {C : Nat → Sort _} {n m k} (hnm : n ≤ m) (hmk : m ≤ k) {next} (x : C n) :
    (leRecOn (Nat.le_trans hnm hmk) (@next) x : C k) = leRecOn hmk (@next) (leRecOn hnm (@next) x) := by
  induction hmk with
...
theorem leRecOn_succ_left {C : Nat → Sort _} {n m} (h1 : n ≤ m) (h2 : n + 1 ≤ m)
    {next : ∀ {k}, C k → C (k + 1)} (x : C n) :
    (leRecOn h2 next (next x) : C m) = (leRecOn h1 next x : C m) := by
  rw [Subsingleton.elim h1 (Nat.le_trans (le_succ n) h2), leRecOn_trans (le_succ n) h2, leRecOn_succ']
...
@[elab_as_elim]
protected def strongRec' {p : Nat → Sort _} (H : ∀ n, (∀ m, m < n → p m) → p n) : ∀ n : Nat, p n
  | n => H n fun m _ ↦ Nat.strongRec' H m
-- #align nat.strong_rec' Nat.strongRec'

/-- Recursion principle based on `<` applied to some natural number. -/
@[elab_as_elim]
def strongRecOn' {P : Nat → Sort _} (n : Nat) (h : ∀ n, (∀ m, m < n → P m) → P n) : P n :=
  Nat.strongRec' h n
-- #align nat.strong_rec_on' Nat.strongRecOn'

theorem strongRecOn'_beta {P : Nat → Sort _} {h} :
...
/-- Induction principle starting at a non-zero number. For maps to a `Sort _` see `leRecOn`.
...
Also works for functions to `Sort _`. For m version assuming only the assumption for `k < n`, see
`decreasing_induction'`. -/
@[elab_as_elim]
def decreasingInduction {P : Nat → Sort _} (h : ∀ n, P (n + 1) → P n) (mn : m ≤ n) (hP : P n) : P m :=
  leRecOn mn (fun {k} ih hsk ↦ ih <| h k hsk) (fun h ↦ h) hP
-- #align nat.decreasing_induction Nat.decreasingInduction

@[simp]
theorem decreasingInduction_self {P : Nat → Sort _} (h : ∀ n, P (n + 1) → P n) (nn : n ≤ n)
    (hP : P n) :
...
theorem decreasingInduction_succ {P : Nat → Sort _} (h : ∀ n, P (n + 1) → P n) (mn : m ≤ n)
    (msn : m ≤ n + 1) (hP : P (n + 1)) :
...
@[simp]
theorem decreasingInduction_succ' {P : Nat → Sort _} (h : ∀ n, P (n + 1) → P n) {m : Nat}
    (msm : m ≤ m + 1) (hP : P (m + 1)) : decreasingInduction h msm hP = h m hP := by
  dsimp only [decreasingInduction]; rw [leRecOn_succ']
-- #align nat.decreasing_induction_succ' Nat.decreasingInduction_succ'

theorem decreasingInduction_trans {P : Nat → Sort _} (h : ∀ n, P (n + 1) → P n)
  ...
  | step hnk ih => rw [decreasingInduction_succ h (Nat.le_trans hmn hnk), ih, decreasingInduction_succ]
-- #align nat.decreasing_induction_trans Nat.decreasingInduction_trans

theorem decreasingInduction_succ_left {P : Nat → Sort _} (h : ∀ n, P (n + 1) → P n)
...
/-- Given `P : Nat → Nat → Sort _`, if for all `m n : Nat` we can extend `P` from the rectangle
strictly below `(m, n)` to `P m n`, then we have `P n m` for all `n m : Nat`.
Note that for non-`Prop` output it is preferable to use the equation compiler directly if possible,
since this produces equation theorems. -/
@[elab_as_elim]
def strongSubRecursion {P : Nat → Nat → Sort _} (H : ∀ m n, (∀ x y, x < m → y < n → P x y) → P m n) :
    ∀ n m : Nat, P n m
  | n, m => H n m fun x y _ _ ↦ strongSubRecursion H x y
-- #align nat.strong_sub_recursion Nat.strongSubRecursion

/-- Given `P : Nat → Nat → Sort _`, if we have `P m 0` and `P 0 n` for all `m n : Nat`, and for any
`m n : Nat` we can extend `P` from `(m, n + 1)` and `(m + 1, n)` to `(m + 1, n + 1)` then we have
`P m n` for all `m n : Nat`.

Note that for non-`Prop` output it is preferable to use the equation compiler directly if possible,
since this produces equation theorems. -/
@[elab_as_elim]
def pincerRecursion {P : Nat → Nat → Sort _} (Ha0 : ∀ m : Nat, P m 0) (H0b : ∀ n : Nat, P 0 n)
    (H : ∀ x y : Nat, P x y.succ → P x.succ y → P x.succ y.succ) : ∀ n m : Nat, P n m
  | m, 0 => Ha0 m
  | 0, n => H0b n
...
Also works for functions to `Sort _`. Weakens the assumptions of `decreasing_induction`. -/
@[elab_as_elim]
def decreasingInduction' {P : Nat → Sort _} {m n : Nat} (h : ∀ k < n, m ≤ k → P (k + 1) → P k)
...
/-- Given a predicate on two naturals `P : Nat → Nat → Prop`, `P a b` is true for all `a < b` if
...
@[elab_as_elim]
theorem diag_induction (P : Nat → Nat → Prop) (ha : ∀ a, P (a + 1) (a + 1)) (hb : ∀ b, P 0 (b + 1))
...
-- /-- A subset of `Nat` containing `k : Nat` and closed under `Nat.succ` contains every `n ≥ k`. -/
-- theorem set_induction_bounded {S : Set Nat} (hk : k ∈ S) (h_ind : ∀ k : Nat, k ∈ S → k + 1 ∈ S)
--     (hnk : k ≤ n) : n ∈ S :=
--   @leRecOn (fun n => n ∈ S) k n hnk @h_ind hk
-- #align nat.set_induction_bounded Nat.set_induction_bounded

-- /-- A subset of `Nat` containing zero and closed under `Nat.succ` contains all of `Nat`. -/
-- theorem set_induction {S : Set Nat} (hb : 0 ∈ S) (h_ind : ∀ k : Nat, k ∈ S → k + 1 ∈ S) (n : Nat) :
--     n ∈ S :=
...
theorem mul_div_mul_comm_of_dvd_dvd (hba : b ∣ a) (hdc : d ∣ c) :
    a * c / (b * d) = a / b * (c / d) := by
...
theorem not_pos_pow_dvd : ∀ {a n : Nat} (_ : 1 < a) (_ : 1 < n), ¬ a ^ n ∣ a
...
theorem div_dvd_of_dvd (h : n ∣ m) : m / n ∣ m := ⟨n, (Nat.div_mul_cancel h).symm⟩
...
@[simp]
theorem mod_succ_eq_iff_lt : m % n.succ = m ↔ m < n.succ :=
...
theorem one_mod_of_ne_one : ∀ {n : Nat}, n ≠ 1 → 1 % n = 1
  | 0, _ | (n + 2), _ => by simp
...
protected theorem dvd_add_left (h : a ∣ c) : a ∣ b + c ↔ a ∣ b := (Nat.dvd_add_iff_left h).symm
-- #align nat.dvd_add_left Nat.dvd_add_left

protected theorem dvd_add_right (h : a ∣ b) : a ∣ b + c ↔ a ∣ c := (Nat.dvd_add_iff_right h).symm
-- #align nat.dvd_add_right Nat.dvd_add_right

protected theorem mul_dvd_mul_iff_left (ha : 0 < a) : a * b ∣ a * c ↔ b ∣ c :=
...
protected theorem mul_dvd_mul_iff_right (hc : 0 < c) : a * c ∣ b * c ↔ a ∣ b :=
...
theorem mul_dvd_of_dvd_div (hcb : c ∣ b) (h : a ∣ b / c) : c * a ∣ b :=
  have ⟨d, hd⟩ := h
...
theorem mul_add_mod_of_lt (h : c < b) : (a * b + c) % b = c := by
...
-- theorem find_eq_iff (h : ∃ n : Nat, p n) : Nat.find h = m ↔ p m ∧ ∀ n < m, ¬ p n := by
--   constructor
--   · rintro rfl
...
-- @[simp] theorem find_lt_iff (h : ∃ n : Nat, p n) (n : Nat) : Nat.find h < n ↔ ∃ m < n, p m :=
...
-- @[simp] theorem find_le_iff (h : ∃ n : Nat, p n) (n : Nat) : Nat.find h ≤ n ↔ ∃ m ≤ n, p m := by
--   simp only [exists_prop, ← Nat.lt_succ_iff, find_lt_iff]
-- -- #align nat.find_le_iff Nat.find_le_iff

-- @[simp] theorem le_find_iff (h : ∃ n : Nat, p n) (n : Nat) : n ≤ Nat.find h ↔ ∀ m < n, ¬ p m := by
--   simp only [← not_lt, find_lt_iff, not_exists, not_and]
-- -- #align nat.le_find_iff Nat.le_find_iff

-- @[simp] theorem lt_find_iff (h : ∃ n : Nat, p n) (n : Nat) : n < Nat.find h ↔ ∀ m ≤ n, ¬ p m := by
--   simp only [← succ_le_iff, le_find_iff, succ_le_succ_iff]
-- -- #align nat.lt_find_iff Nat.lt_find_iff

-- @[simp] theorem find_eq_zero (h : ∃ n : Nat, p n) : Nat.find h = 0 ↔ p 0 := by simp [find_eq_iff]
...
-- theorem find_pos (h : ∃ n : Nat, p n) : 0 < Nat.find h ↔ ¬p 0 :=
...
-- theorem findGreatest_succ (n : Nat) :
...
-- theorem findGreatest_le (n : Nat) : Nat.findGreatest P n ≤ n :=
...
-- theorem findGreatest_mono_right (P : Nat → Prop) [DecidablePred P] {m n} (hmn : m ≤ n) :
...
-- theorem findGreatest_mono_left [DecidablePred Q] (hPQ : ∀ n, P n → Q n) (n : Nat) :
...
instance decidableLoHi (lo hi : Nat) (P : Nat → Prop) [H : DecidablePred P] :
