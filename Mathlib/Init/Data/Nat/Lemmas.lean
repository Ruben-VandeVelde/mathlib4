...
variable {m n : ℕ} {q : ℕ → Prop} [DecidablePred q]
...
@[simp] theorem find_eq_zero (h : ∃ n : ℕ, p n) : Nat.find h = 0 ↔ p 0 := by
  simp only [find_eq_iff, not_lt_zero, false_implies, implies_true, and_true]
...
@[simp] theorem findGreatest_pos : 0 < Nat.findGreatest P k ↔ ∃ n, 0 < n ∧ n ≤ k ∧ P n := by
  ...
  simp only [Classical.not_forall, Classical.not_imp, Decidable.not_not, exists_prop]
